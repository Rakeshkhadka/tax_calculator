def calculateIncome(salary, salaryType, festival=0, allowance=0, others=0):
    festival = 0 if festival == None else festival
    allowance = 0 if allowance == None else allowance
    others = 0 if others == None else others
...
def calculateDeduction(income, pf=0,  cit=0, ssf=0, li=0, mi=0):
    pf = 0 if pf == None else pf
    cit = 0 if cit == None else cit
    ssf = 0 if ssf == None else ssf
    li = 0 if li == None else li
    mi = 0 if mi == None else mi
    if pf != 0 and ssf != 0:
        raise ValueError("You can't contribute on both Provident Fund and Social Security Fund.")
    if pf > (income*20/100):
        raise ValueError("Provident Fund must not be greater than 20% of total income.")
    if cit > 300000:
        raise ValueError("Contribution mustn't be greater than 300000 per year.")
